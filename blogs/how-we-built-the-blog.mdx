---
title: 'How We Built Our Blog: Astro, MDX, and Almost No JavaScript'
description: 'Astro, MDX, and GitHub Pages, with React islands that only load when you scroll to them. The best decisions are the things we left out.'
pubDate: 2026-07-30
tags: ['astro', 'mdx', 'web', 'github-pages']
socialPost: 'We have a pile of projects in various states of barely to mostly done, so we started a blog to force ourselves to finish things. First post: how we built it — almost zero JavaScript, React islands that load on scroll, and components written by a coding agent.'
social:
  twitter:
    - "1/ We have a pile of projects in various states of barely to mostly done. So we built a blog with one rule: nothing gets posted until it's finished. First post: how we built it."
    - '2/ The site: Astro + MDX, hosted on GitHub Pages. Almost every page ships zero JavaScript — just pre-built HTML, unless something genuinely needs interactivity.'
    - "3/ The things that need interactivity are React islands. They only download and hydrate once you scroll them into view. There's one embedded in the middle of the post, and it's live, not a screenshot."
    - "4/ Why most blogs don't do this: every interactive component is a mini React app, and they're tedious to hand-code. So a coding agent writes ours. A build script then rejects anything with eval, fetch, or dynamic imports, and we review the result like any other PR."
    - "5/ The decision we're proudest of is the stuff we never built: no comments, no tracking, no client-side router, no framework running the whole site. Boring is the goal. Boring usually just works."
    - 'Full writeup, with a live component embedded in the prose:'
  bluesky:
    - 'We have a pile of projects in various states of barely to mostly done, so we built a blog to force ourselves to finish things. First post: how we built it.'
    - 'Almost every page ships zero JavaScript. Interactivity lives in React islands that only load on scroll — one is live in the middle of this post. The components are written by a coding agent, then checked by a lint script before we merge.'
  mastodon: 'We have a pile of projects in various states of barely to mostly done, so we built a blog to force ourselves to finish things. First post: how we built it. Astro + MDX on GitHub Pages, almost zero JavaScript, React islands that load on scroll (one is live in the post), components written by a coding agent then linted before merge. Proudest of what we left out: no comments, no tracking, no client-side router.'
  linkedin: |
    We have a long list of projects in various states of barely to mostly done. The blog is our fix: nothing gets published until it's actually finished. A clean breakpoint, an artefact, done.

    Naturally, the first post is about the blog itself. The stack is Astro 5 with MDX, React 19 for the few interactive parts, Tailwind 4, and GitHub Pages for hosting. Almost every page ships zero JavaScript — when something genuinely needs interactivity, it becomes a React island that only downloads and hydrates once you scroll it into view.

    The part people find surprising: the interactive components are written by a coding agent. We didn't hand-code them, and we don't blindly trust them either — a build script rejects anything with eval, external fetches, or dynamic imports, a strict CSP contains whatever's left, and a human reviews every component before merge.

    And the decision we're proudest of is everything we didn't build: no comment system, no tracking, no client-side router, no framework running the whole site. Each of those is maintenance we never do.

    Full write-up, with a live component embedded in the prose:
---
...
**Maths via [KaTeX](https://katex.org/)** (`remark-math` + `rehype-katex`). Inline math like $E = mc^2$ renders mid-sentence, and block math gets its own centred display. For example, the attention-cost equation from a blog we are working on appears like this:
