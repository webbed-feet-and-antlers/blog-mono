---
title: 'How We Built Our Blog with Astro, MDX and interactive islands'
description: 'Astro statically generated site, using MDX for AI generated interactive posts, with React islands that only loads when you scroll to them, all hosted for free on GitHub Pages.'
pubDate: 2026-08-27
tags: ['astro', 'mdx', 'web', 'github-pages']
socialPost: 'We have a pile of projects in various states of barely to mostly done, so we started a blog to force ourselves to finish things. Our first post: how we built it, with Astro and interactive React islands for interactive components built with AI agents.'
social:
  twitter:
    - '1/ We have a pile of projects in various states of barely to mostly done. So we built a blog to force us to finish stuff. First post: how we built it.'
    - '2/ The site: Astro + MDX, hosted on GitHub Pages. The site is static, just pre-built HTML, unless something genuinely needs interactivity to explain a point.'
    - "3/ Interactive components are React islands. They only download and hydrate once you scroll them into view. They’re embedded in the posts, they're not just another screenshot."
    - '4/ Every interactive component is a mini React app, so it would be annoying to hand-code. We built a harness for an AI agent to build them for us, it rejects anything unsafe, and we can review the result like any other PR.'
    - 'Full writeup, with embedded live components:'
  bluesky:
    ...
    - "It's a static Astro site with interactive React islands for explainers that only load on scroll. The components are written by an AI agent, then automatically checked and verified."
  mastodon: 'We have a pile of projects in various states of barely to mostly done, so we built a blog to force ourselves to finish things. Our first post: how we built it. Astro + MDX, hosted on GitHub Pages, a static site with interactive React islands explainers that load on scroll, and interactive components written by a coding agent then automatically tested and verified.'
  linkedin: |
    We have a long list of projects in various states of barely to mostly done. So we built our blog to fix this, nothing gets published until it's actually finished and a post is a clean final output.

    Our first post is about the blog itself. The stack is Astro 5 with MDX, client side React for the few interactive parts, Tailwind 4, and GitHub Pages for hosting. We include interactive explainers embedded within the blog, which are React islands that only downloads and hydrates once you scroll it into view. We added agent skills and a small harness to build the interactive components with AI agents. This rejects anything unsafe, runs a strict CSP to contain whatever's left.

    Full write-up, with a live component embedded in the prose:
syndication:
  devto: "4494647"
  bluesky: at://did:plc:nquupi7xh3s47nk5bjqmynt6/app.bsky.feed.post/3mtyd73vett2x
  buffer: 6a8edace2976fdf632ec700a
  mastodon: "117161909434268271"
  substack: https://theinkpens.substack.com/p/how-we-built-our-blog-with-astro
draftLinks:
  linkedinArticle: https://www.linkedin.com/article/edit/7498380621434544128/
---
